/**
  ...
  *          ===================================================================
  *           This driver manages the "Universal Serial Bus C
  *             - Union Functional collection (using 1 IN endpoinlass Definitions for Communications Devices
  ...
  *             - Abstract Control Model compliantt for control)
...
__ALIGN_BEGIN static uint8_t USBD_CDC_CfgHSDesc[] __ALIGN_END =
{
  ...
  0x00,                                       /* bAlternateSetting: Alternate setting */
  0x04,                                       /* bNumEndpoints: Two endpoints used */
...
__ALIGN_BEGIN static uint8_t USBD_CDC_CfgFSDesc[] __ALIGN_END =
{
  ...
  67+14-35,/* wTotalLength:no of returned bytes */
  0x00,
...
__ALIGN_BEGIN static uint8_t USBD_CDC_OtherSpeedCfgDesc[] __ALIGN_END =
{
  0x09,                                       /* bLength: Configuration Descriptor size */
...
uint8_t USBD_CDC_TransmitPacket(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (hcdc->TxState[(epnum&0x0F)-1] == 0U)
  {
    /* Tx Transfer in progress */
    hcdc->TxState[(epnum&0x0F)-1] = 1U;

    /* Update the packet total length */
    pdev->ep_in[epnum & 0xFU].total_length = hcdc->TxLength[(epnum & 0xFU)-1];

    /* Transmit next packet */
    (void)USBD_LL_Transmit(pdev, epnum, hcdc->TxBuffer[(epnum&0x0F)-1], hcdc->TxLength[(epnum&0x0F)-1]);
